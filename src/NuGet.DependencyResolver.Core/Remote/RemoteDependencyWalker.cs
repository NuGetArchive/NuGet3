...
namespace NuGet.DependencyResolver
{
    public class RemoteDependencyWalker
    {
        ...
        public static bool IsGreaterThanOrEqualTo(VersionRange nearVersion, VersionRange farVersion)
        {
            ...
            else if (nearVersion.IsFloating || farVersion.IsFloating)
            {
                ...
                if (nearVersion.IsFloating)
                {
                    ...
                    nearMinVersion = GetReleaseLabelFreeVersion(
                        nearVersion,
                        farVersion.IsFloating ? farVersion.Float.FloatBehavior : NuGetVersionFloatBehavior.None);
                ...
                if (farVersion.IsFloating)
                {
                    ...
                    farMinVersion = GetReleaseLabelFreeVersion(
                        farVersion,
                        nearVersion.IsFloating ? nearVersion.Float.FloatBehavior : NuGetVersionFloatBehavior.None);
        ...
        private static NuGetVersion GetReleaseLabelFreeVersion(VersionRange versionRange, NuGetVersionFloatBehavior otherFloatBehavior)
        {
            if (versionRange.Float.FloatBehavior == NuGetVersionFloatBehavior.Major ||
                otherFloatBehavior == NuGetVersionFloatBehavior.Major)
            {
                return new NuGetVersion(0, 0, 0);
            }
            else if (versionRange.Float.FloatBehavior == NuGetVersionFloatBehavior.Minor ||
                otherFloatBehavior == NuGetVersionFloatBehavior.Minor)
            {
                return new NuGetVersion(versionRange.MinVersion.Major, 0, 0);
            }
            else if (versionRange.Float.FloatBehavior == NuGetVersionFloatBehavior.Patch ||
                otherFloatBehavior == NuGetVersionFloatBehavior.Patch)
            {
                return new NuGetVersion(versionRange.MinVersion.Major, versionRange.MinVersion.Minor, 0);
            }
            else if (versionRange.Float.FloatBehavior == NuGetVersionFloatBehavior.Revision ||
                otherFloatBehavior == NuGetVersionFloatBehavior.Revision)
            {
                return new NuGetVersion(versionRange.MinVersion.Major, versionRange.MinVersion.Minor, versionRange.MinVersion.Patch);
            }
            else
            {
